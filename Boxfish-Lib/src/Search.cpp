...
#define BOX_UNDO_MOVES 0
	...
	ValueType Search::SearchPosition(Position& position, SearchStack* stack, int depth, ValueType alpha, ValueType beta, int& selDepth, bool cutNode, const Search::RootInfo& rootInfo)
	{
		...
		bool ttFound;
		TranspositionTableEntry* ttEntry = m_TranspositionTable.GetEntry(ttHash, ttFound);
		...
		{
			const bool ttEntryValid = (ttValue >= beta) ? (ttEntry->GetFlag() & LOWER_BOUND) : (ttEntry->GetFlag() & UPPER_BOUND);
			if (ttEntryValid)
			{
				if (ttMove != MOVE_NONE)
				{
		...
		if (!inCheck)
		{
			if (ttFound && ttEntry->GetDepth() >= depth - 1 && (ttEntry->GetFlag() & LOWER_BOUND))
			{
		...
		while ((move = selector.GetNextMove()) != MOVE_NONE)
		{
			...
			if (value >= beta)
			{
				...
				{
					ttEntry->Update(position.Hash, move, depth, GetValueForTT(value, stack->Ply), LOWER_BOUND, position.GetTotalHalfMoves(), IsPvNode);
				}
				return value;
			}
	...
	ValueType Search::QuiescenceSearch(Position& position, SearchStack* stack, int depth, ValueType alpha, ValueType beta)
	{
		...
		if (!inCheck)
		{
			...
			if (evaluation >= beta && !inCheck)
			{
				if (!ttFound || ReplaceTT(depth, position.GetTotalHalfMoves(), LOWER_BOUND, ttEntry))
				{
					ttEntry->Update(position.Hash, MOVE_NONE, depth, GetValueForTT(evaluation, stack->Ply), LOWER_BOUND, position.GetTotalHalfMoves(), IsPvNode);
				}
				return evaluation;
			}
			if (alpha < evaluation)
		...
		while (!selector.Empty())
		{
			...
			if (generator.IsLegal(move))
			{
				...
				if (score >= beta)
				{
					if (!ttFound || ReplaceTT(depth, position.GetTotalHalfMoves(), LOWER_BOUND, ttEntry))
					{
						ttEntry->Update(position.Hash, move, depth, GetValueForTT(score, stack->Ply), LOWER_BOUND, position.GetTotalHalfMoves(), IsPvNode);
					}
					return score;
				}
				if (score > alpha)
		...
		if (!ttFound || ReplaceTT(depth, position.GetTotalHalfMoves(), entryFlag, ttEntry))
		{
			ttEntry->Update(position.Hash, MOVE_NONE, depth, GetValueForTT(alpha, stack->Ply), entryFlag, position.GetTotalHalfMoves(), IsPvNode);
		}
