...
namespace Test
{
    ...
    constexpr int TestCount = sizeof(TestFens) / sizeof(char*) / 2;

    TEST_CASE("WinAtChess", "[WinAtChess]")
    {
        Init();
        ...
        {
            std::string fen = TestFens[i * 2 + 0];
            std::string expectedMoves = TestFens[i * 2 + 1];

            size_t space = expectedMoves.find(' ');
            std::string pgnMove;
            if (space != std::string::npos)
                pgnMove = expectedMoves.substr(0, space);
            else
                pgnMove = expectedMoves;
            std::cout << "FEN: " << fen << std::endl;
            currentPosition = CreatePositionFromFEN(fen);
            SearchLimits limits;
            limits.Milliseconds = searchTime;
            Move bestMove = search.SearchBestMove(currentPosition, limits);
            if (PGN::FormatMove(bestMove, currentPosition) == pgnMove)
                passed++;
            else
                failed++;
            search.Reset();
